...
class AttackDao(metaclass=Singleton):
    def find_all_attacks(self, limit:int=0, offest:int=0) -> List[AbstractAttack]:
        ...
        :type offest: int
        """
        request = "SELECT * FROM attack JOIN attack_type USING(id_attack_type)"
        if limit :
    ...
    def find_attack_by_id(self,id_attack:int)-> Optional[AbstractAttack]:
        ...
        with DBConnection().connection as connection:
            with connection.cursor() as cursor :
                cursor.execute(
                    "SELECT * from attack JOIN attack_type "\
                    "USING(id_attack_type) WHERE id_attack=%(id)s"
                , {"id" : id_attack})
                res = cursor.fetchone()
        attack = None
        attack_factory = AttackFactory()
    ...
    def add_attack(self, attack : AbstractAttack) -> bool:
        ...
        with DBConnection().connection as connection:
            with connection.cursor() as cursor :
                cursor.execute(
                    "INSERT INTO attack (id_attack_type, attack_name,"\
                    " power, accuracy, element, attack_description) VALUES "\
                    "(%(id_type)s, %(name)s, %(power)s, %(accuracy)s, %(element)s, %(attack_description)s)"\
                    "RETURNING id_attack;"
                , {"id_type" : id_type
                  , "name": attack.name
                  , "power": attack.power
